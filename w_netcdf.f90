...
MODULE write_netcdf
  ...
  sizes_ex = SHAPE(Ex)
  sizes_ey = SHAPE(Ey)
  sizes_a = SHAPE(elle%a)
  sizes_v = SHAPE(elle%v)
  sizes_r = SHAPE(elle%r)
  ...
  END IF  
  !Electron position
  ierr = nf90_put_var(file_id, var_id_r, elle%r)
  IF (ierr /= nf90_noerr) THEN
    PRINT *, TRIM(nf90_strerror(ierr))
    RETURN
  END IF  
  !Electron velocity
  ierr = nf90_put_var(file_id, var_id_v, elle%v) 
  IF (ierr /= nf90_noerr) THEN
    PRINT *, TRIM(nf90_strerror(ierr))
    RETURN
  END IF  
  !Electron acceleration
  ierr = nf90_put_var(file_id, var_id_a, elle%a)   
  IF (ierr /= nf90_noerr) THEN
    PRINT *, TRIM(nf90_strerror(ierr))
    RETURN
  END IF
